...
class SemanticAnalysis:
    ...
    def endalismClue(self, clue, fillLength):
        ...
        if 'last' in clueWords:
            ...
            indexPossWord = clueWords.index('final') + 1
    ...
    def mostProbableWords(self, fillLength, clue, fillType):
        ...
        for t in typeClue:
            print 'currently testing: ' + t + ' type of clue'
        ...
        if fillLength in self.lenToCrossWordiest:
    ...
    #     chilepine
    def getTestData(self, test_filename):
        f = open(test_filename)
        lines = f.readlines()
        testData = {} # a dict mapping type of clue to (clue, answer) pairs
        clue = ''
        answer = ''
        typeOfClue = ''
        ...
        for i in range(len(lines)): 
            if i % 3 == 0:  
                typeOfClueIndex = lines[i].index('-') + 1
                typeOfClue = lines[i][typeOfClueIndex:].strip()
            elif i % 3 == 1:
                clue = lines[i].strip()

            elif i % 3 == 2:
                answer = lines[i].strip()
                if typeOfClue in testData:
                    testData[typeOfClue].append((clue, answer))
                else:
                    testData[typeOfClue] = [(clue,answer)]
    ...
    def test(self, testData):
        acrossClues = testData['across']
        downClues = testData['down']
        totalSuccesses = 0
        totalClues = 0

        for clueAnswerPair in downClues:
            print "actual clue answer combo: ", clueAnswerPair
            fillLength = len(clueAnswerPair[1])
            clue = clueAnswerPair[0]
            answer = clueAnswerPair[1]
            fillType = 'down'
            mostProbableWords = self.mostProbableWords(fillLength, clue, fillType)
            if answer in mostProbableWords:
                totalSuccesses += 1
            totalClues += 1
            print "most probable words alg comes up with: ", mostProbableWords

        print "total successes: " + str(totalSuccesses)
        print "total clues: " + str(totalClues)
...
def main():
    ...
    testData = sa.getTestData("cryptic-clues-testData.txt")
    sa.test(testData)
