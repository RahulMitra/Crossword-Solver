...
def orderValues(variable, domains, cluesToWords, lengthToWords, wordFreqs, answerMap): 
    length = variable.length ## NOTE, THIS IS NOT IMPLEMENTED YET IN CSP SHIT AND NEEDS TO BE UPDATED
    ...
    wordOfLenN = lengthToWords[length]
    for answer in wordOfLenN:
        # Compute score
        features = generateFeatureVector(clue, answer, lengthToWords, wordFreqs, answerMap)
    ...
    return sorted(answerToScore.items(), key=operator.itemgetter(1))
